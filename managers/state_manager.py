        ...
        try:
            if self.state == "MEASURE":
                if self.command == "pH":
                    ...
                    ble_comm.send('{"NAME":"pH","VALUE":%.3f,"UNITS":"%s"}\r\n' % (ph[0], ph[1]))
                    ...
                    ble_comm.send('{"NAME":"CONDUCTIVIDAD","VALUE":%.3f,"UNITS":"%s"}\r\n' % (conductivity[0], conductivity[1]))
                    ...
                    ble_comm.send('{"NAME":"TURBIEDAD","VALUE":%.3f,"UNITS":"%s"}\r\n' % (turbidity[0], turbidity[1]))
                    ...
                    ble_comm.send('{"NAME":"BATTERY","VALUE":%d,"UNITS":"%s"}\r\n' % (battery[0], battery[1]))
